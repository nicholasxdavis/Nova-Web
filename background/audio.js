document.addEventListener("DOMContentLoaded", () => {
    ...
    downloadButton.addEventListener("click", downloadConvertedAudios);
    ...
    async function convertAudios(files) {
    ...
    function simulateConversion(file) {
        return new Promise(resolve => {
            setTimeout(() => {
                resolve(new Blob([file], { type: `audio/${formatSelect.value.toLowerCase()}` }));
            }, 2000);
        });
    }
    ...
    async function downloadConvertedAudios() {
        ...
        if (convertedBlobs.length === 1) {
            ...
            a.href = url;
            a.download = `converted_audio_1.${formatSelect.value.toLowerCase()}`;
        ...
        if (convertedBlobs.length > 1) {
            try {
                console.log("Downloading converted audios...");
                const zip = new JSZip();

                convertedBlobs.forEach((blob, index) => {
                    const baseFileName = `converted_audio_${index + 1}`;
                    const finalFileName = `${baseFileName}.${formatSelect.value.toLowerCase()}`;
                    zip.file(finalFileName, blob);
                });

                const zipBlob = await zip.generateAsync({ type: "blob" });
                const downloadLink = document.createElement("a");
                const zipFileName = "converted_audios.zip";
            ...
            alert("No converted audios found. Please convert some audios first.");
        }
    }
});